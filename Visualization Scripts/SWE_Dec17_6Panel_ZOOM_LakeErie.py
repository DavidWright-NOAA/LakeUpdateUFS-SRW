...
xpos = 0.03 #.08 Panel Label location
ypos = 0.105 #.15

### Zoom Region for Map
lonMin_Z = -83.8 #Lake Huron: -85.1 #Lake Erie:-83.8 #Lake Michigan: -88.0# # #-92.5#-94.0#-82.083####
lonMax_Z = -76.9 #Lake Huron: -79.1 #Lake Erie:-75.0 #Lake Michigan: -84.0# # #-84.0# -74.0#-77.695####
latMin_Z = 40.8 #Lake Huron: 42.8 #Lake Erie: 41.0 #Lake Michigan: 41.5# # #45.2#40.0#41.012####
latMax_Z = 43.45 #Lake Huron: 46.5 #Lake Erie: 44.8 #Lake Michigan: 44.5# # #49.3#50.0#43.469####
 
img_extent = [lonMin_Z, lonMax_Z, latMin_Z, latMax_Z] 

### Color map and range
cmap = plt.cm.jet
cLevels = np.arange(0.0,32.0,2)
norm = BoundaryNorm(cLevels, ncolors=cmap.N, clip=True)

cmap_diff = plt.cm.bwr
cLevels_diff = [-8.,-6.,-4.,-2.,-1.,1.,2.,4.,6.,8.]#[-10.,-8.,-6.,-4.,-2.,-1.,1.,2.,4.,6.,8.,10.] #np.arange(-2.0,2.2,.2)
norm_diff = BoundaryNorm(cLevels_diff, ncolors=cmap_diff.N, clip=True)

### File Paths and File Loads
base_fp = ''

swe_ctl_Ids = nc.Dataset(base_fp + 'UFS-SRW/2017122412_CTRL/phyf018.nc')
swe_ctl_Fds = nc.Dataset(base_fp + 'UFS-SRW/2017122412_CTRL/phyf042.nc')

swe_sta_Ids = nc.Dataset(base_fp + 'UFS-SRW/2017122412_STAT/phyf018.nc')
swe_sta_Fds = nc.Dataset(base_fp + 'UFS-SRW/2017122412_STAT/phyf042.nc')

swe_dyn_Ids = nc.Dataset(base_fp + 'UFS-SRW/2017122412_DYN/phyf018.nc')
swe_dyn_Fds = nc.Dataset(base_fp + 'UFS-SRW/2017122412_DYN/phyf042.nc')

snodas_ds = nc.Dataset(base_fp + 'SNODAS/SNODAS_20171226-20171225_DIFF_SWE.nc')
...
cb2 = fig.colorbar(outplt2, ax=axlist[5], cax=cax2, orientation='vertical', ticks=[-8,-6.,-4.,-2.,-1.,1.,2.,4.,6.,8.]) #ticks=[-10.,-8.,-6.,-4.,-2,-1.,1.,2.,4.,6.,8.,10.])#, ticks=cLevels[::2]) #, location = 'bottom')#, shrink=.8)
cb2.set_label(label=r'${mm}$', size=8)
cb2.ax.tick_params(labelsize=8)

plt.subplots_adjust(hspace = -0.3, wspace=0.0) #left=0.05,bottom=.1,top=.95,right=0.99,
#plt.show()
plt.savefig('Publication/Figures/SWE_Dec2017_6panels_ZOOM_LakeErie.png', dpi=1800, bbox_inches='tight')