...
MH_Divide = -84.7540+360. #Dividing longitude taken from Fujisaki-Manome et al. 2020 paper to divide Michigan and Huron
...
for file in ['Dec2017_lh_spatialMean.txt','Feb2021_lh_spatialMean.txt','Jan2022_lh_spatialMean.txt','Nov2022_lh_spatialMean.txt']:
    with open(file,'r', newline = '\n') as fp:
        ...
        for x in all_lines:
            ...
            mi_ctl_lh.append(float(x[1]))
            mi_sta_lh.append(float(x[2]))
            mi_dyn_lh.append(float(x[3]))
    
            hu_ctl_lh.append(float(x[4]))
            hu_sta_lh.append(float(x[5]))
            hu_dyn_lh.append(float(x[6]))
    
            su_ctl_lh.append(float(x[7]))
            su_sta_lh.append(float(x[8]))
            su_dyn_lh.append(float(x[9]))
    
            er_ctl_lh.append(float(x[10]))
            er_sta_lh.append(float(x[11]))
            er_dyn_lh.append(float(x[12]))
    
            on_ctl_lh.append(float(x[13]))
            on_sta_lh.append(float(x[14]))
            on_dyn_lh.append(float(x[15]))            
    
    
    ###Take differences between values, convert lists to numpy array for easy subtraction
    mi_ctl_lh = np.array(mi_ctl_lh)
    mi_sta_lh = np.array(mi_sta_lh)
    mi_dyn_lh = np.array(mi_dyn_lh)
    
    hu_ctl_lh = np.array(hu_ctl_lh)
    hu_sta_lh = np.array(hu_sta_lh)
    hu_dyn_lh = np.array(hu_dyn_lh)
    
    su_ctl_lh = np.array(su_ctl_lh)
    su_sta_lh = np.array(su_sta_lh)
    su_dyn_lh = np.array(su_dyn_lh)
    
    er_ctl_lh = np.array(er_ctl_lh)
    er_sta_lh = np.array(er_sta_lh)
    er_dyn_lh = np.array(er_dyn_lh)
    
    on_ctl_lh = np.array(on_ctl_lh)
    on_sta_lh = np.array(on_sta_lh)
    on_dyn_lh = np.array(on_dyn_lh)
    
    mi_sta_ctl.append(mi_sta_lh - mi_ctl_lh)  
    mi_dyn_sta.append(mi_dyn_lh - mi_sta_lh)
    
    hu_sta_ctl.append(hu_sta_lh - hu_ctl_lh)
    hu_dyn_sta.append(hu_dyn_lh - hu_sta_lh)
    
    su_sta_ctl.append(su_sta_lh - su_ctl_lh)
    su_dyn_sta.append(su_dyn_lh - su_sta_lh)
    
    er_sta_ctl.append(er_sta_lh - er_ctl_lh)
    er_dyn_sta.append(er_dyn_lh - er_sta_lh)
    
    on_sta_ctl.append(on_sta_lh - on_ctl_lh)
    on_dyn_sta.append(on_dyn_lh - on_sta_lh)
...
plt.savefig('/Publication/Figures/LH_Line_20Panel_ROTATED.png', dpi=1800)
