...
fp = open('Feb2021_sh_spatialMean.txt','w')
...
for fhr in np.arange(0,55,1):
    ...
    ctl_lh = ctl_ds.select(name='Sensible heat net flux', typeOfLevel='surface', forecastTime=fhr)[0].values
    sta_lh = sta_ds.select(name='Sensible heat net flux', typeOfLevel='surface', forecastTime=fhr)[0].values
    dyn_lh = dyn_ds.select(name='Sensible heat net flux', typeOfLevel='surface', forecastTime=fhr)[0].values
