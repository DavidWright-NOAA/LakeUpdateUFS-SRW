...
xpos = 0.047 #Panel Label location
ypos = 0.074

### Zoom Region for Map
lonMin_Z = -87.5 #Lake Huron: -85.1 #Lake Erie:-83.8 #Lake Michigan: -88.0# # #-92.5#-94.0#-82.083####
lonMax_Z = -85.0 #Lake Huron: -79.1 #Lake Erie:-75.0 #Lake Michigan: -84.0# # #-84.0# -74.0#-77.695####
latMin_Z = 41.3 #Lake Huron: 42.8 #Lake Erie: 41.0 #Lake Michigan: 41.5# # #45.2#40.0#41.012####
latMax_Z = 43.5 #Lake Huron: 46.5 #Lake Erie: 44.8 #Lake Michigan: 44.5# # #49.3#50.0#43.469####
...
cLevels_diff = [-10.,-8.,-6.,-4.,-2.,2.,4.,6.,8.,10.] #np.arange(-2.0,2.2,.2)
norm_diff = BoundaryNorm(cLevels_diff, ncolors=cmap_diff.N, clip=True)

### File Paths and File Loads
base_fp = ''

swe_ctl_Ids = nc.Dataset(base_fp + 'UFS-SRW/22010512_CTRL/phyf018.nc')
swe_ctl_Fds = nc.Dataset(base_fp + 'UFS-SRW/22010512_CTRL/phyf066.nc')

swe_sta_Ids = nc.Dataset(base_fp + 'UFS-SRW/22010512_STAT/phyf018.nc')
swe_sta_Fds = nc.Dataset(base_fp + 'UFS-SRW/22010512_STAT/phyf066.nc')

swe_dyn_Ids = nc.Dataset(base_fp + 'UFS-SRW/22010512_DYN/phyf018.nc')
swe_dyn_Fds = nc.Dataset(base_fp + 'UFS-SRW/22010512_DYN/phyf066.nc')

snodas_ds = nc.Dataset(base_fp + 'SNODAS/SNODAS_20220108-20220106_DIFF_SWE.nc')
...
#cax1 = axlist[3].inset_axes([0.5,-.15,2.0,.1]) #[X-location, Y-Location, Width, Height]  https://matplotlib.org/stable/gallery/subplots_axes_and_figures/colorbar_placement.html
#cax2 = axlist[3].inset_axes([0.5,-.6,2.0,.1])
...
cb2 = fig.colorbar(outplt2, ax=axlist[5], cax=cax2, orientation='vertical', ticks=[-10.,-8.,-6.,-4.,-2,2.,4.,6.,8.,10.])#, ticks=cLevels[::2]) #, location = 'bottom')#, shrink=.8)
cb2.set_label(label=r'${mm}$', size=8)
cb2.ax.tick_params(labelsize=8)

plt.subplots_adjust(hspace = 0.3, wspace=-.632) #left=0.05,bottom=.1,top=.95,right=0.99,
#plt.show()
plt.savefig('Figures/SWE_Jan2022_6panels_ZOOM_LakeMichigan.png', dpi=1800, bbox_inches='tight')