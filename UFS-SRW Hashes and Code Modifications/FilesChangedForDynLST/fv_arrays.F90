...
!-----------------------------------------------------------------------
! Parameters for updating lower boundary condtions
! Added by David.M.Wright
!-----------------------------------------------------------------------
  logical :: lowbcupdate = .false. !< If True, then will update low bc
  integer :: lowbc_int = 3 !< Look for new file every X hours. Hours selected as units to keep consitent with bc_update_interval
  character(len=128) :: lowbc_dir = 'INPUT/' !< Where to look for files
  character(len=128) :: lowbc_base = 'lowbc_f' !<Base name of files to search
...
! Added by David.M.Wright to include land-water-ice mask, ice temperature, and
! fractional ice
! NEED TO ADD ICE THICKNESS!!! Either vice or hice
    real, _ALLOCATABLE :: slmsk(:,:)    _NULL  !< Land = 1, Water = 0, Ice =2
    real, _ALLOCATABLE :: tisfc(:,:)    _NULL  !< Ice surface temperature
    real, _ALLOCATABLE :: fice(:,:)     _NULL  !< Fractional Sea/Lake Ice
    real, _ALLOCATABLE :: lkmsk(:,:)    _NULL  !< Lake Mask for NA Great LAkes
...
!! small arrays defined as null 3D arrays.
  subroutine allocate_fv_atmos_type(Atm, isd_in, ied_in, jsd_in, jed_in, is_in, ie_in, js_in, je_in, &
    ...
    ! Added by David.M.Wright, ADD ICE THICKNESS
    allocate ( Atm%slmsk(is:ie,js:je) ) ; Atm%slmsk=real_big!snan
    allocate ( Atm%tisfc(is:ie,js:je) ) ; Atm%tisfc=real_big!snan
    allocate ( Atm%fice(is:ie,js:je) ) ; Atm%fice=real_big!snan
    allocate ( Atm%lkmsk(is:ie,js:je) ) ; Atm%lkmsk=real_big!snan
...
!>@brief The subroutine 'deallocate_fv_atmos_type' deallocates the fv_atmos_type.
  subroutine deallocate_fv_atmos_type(Atm)
    ...
    deallocate (  Atm%ak )
    deallocate (  Atm%bk )
    ! Added by David.M.Wright, ADD HICE
    deallocate (  Atm%ts )
    deallocate (  Atm%slmsk )
    deallocate (  Atm%tisfc )
    deallocate (  Atm%fice )
    deallocate (  Atm%lkmsk )
