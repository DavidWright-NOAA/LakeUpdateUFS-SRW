...
module fv_regional_mod
   ...
   implicit none
      ...
      public ak_in, bk_in                                               &
            ...
            ,write_full_fields                                          &
            ,update_lowbc                                               &!Added by David.M.Wright
            ,read_lowbc_data                                            &
            ,ntimesteps_per_lowbc_update
...
!
      ...
      integer,save :: ntimesteps_per_lowbc_update                          !David.M.Wright
!
...
!-----------------------------------------------------------------------
!
      subroutine update_lowbc(Atm, Time, Time_step_atmos, p_split)!       &
!                      ,isc,iec,jsc,jec                                  &
!                      ,isd,ied,jsd,jed )
!
!-----------------------------------------------------------------------
!***  Update lower boundary condition (specifically lake temperature)
!***  with model run offline. This will take in new data every X
!***  number of minutes and update the skin/surface fields.
!***
!***  Created by David.M.Wright, April 2020
!-----------------------------------------------------------------------
!
      implicit none
!
...
!
!-----------------------------------------------------------------------
!***  Check time to see if new lower bc should be read in
!-----------------------------------------------------------------------
!
...
!      file_name_lbc='INPUT/fvcom_122417_f000.nc'
!      if (mpp_pe() == 0) write(*,"('current_time_seconds_lowbc =
!      ',f9.1)")current_time_in_seconds
!
      call get_time (Time_step_atmos, sec)
      dt_atmos = real(sec)
!
      if(atmos_time_step==0.or.Atm%flagstruct%warm_start)then
        ntimesteps_per_lowbc_update=nint(Atm%flagstruct%lowbc_int*3600./(dt_atmos/real(abs(p_split))))
      endif
!
      if (mpp_pe() == 0) then
        write(*,"('current_time_seconds_lowbc = ',f9.1)")current_time_in_seconds
!        write(*,"('atmos_time = ',f9.1)")atmos_time
!        write(*,"('atmos_time_step = ',f9.1)")atmos_time_step
!        write(*,"('nts per update = ',f9.1)")ntimesteps_per_lowbc_update
      endif
!
!
...
!-------------------------------------------------------------------------
!*** Do this on the first time step to initialize new ice field?
!-------------------------------------------------------------------------
      if(atmos_time_step == 0)then
        if(mpp_pe() == 0) write(*,"('Calling read_lowbc_data at first tstep',f9.1)")current_time_in_seconds
        call read_lowbc_data(Atm)
      endif
!
!
      if(atmos_time_step+1>=ntimesteps_per_lowbc_update.and.mod(atmos_time_step,ntimesteps_per_bc_update)==0)then
        if (mpp_pe() == 0) write(*,"('Calling read_lowbc_data, current_time = ',f9.1)")current_time_in_seconds
        call read_lowbc_data(Atm)
      endif  !<--- End of If statement if the time is correct to update lowbc
!
      end subroutine update_lowbc
!
!-----------------------------------------------------------------------
!
      subroutine read_lowbc_data(Atm)
!***********************************************************************
! Read in and apply lower boundary conditions to Atm
!  David.M.Wright, May 2020
!***********************************************************************
      implicit none
!
      type(fv_atmos_type),target,intent(inout) :: Atm
!<-- Atm object for the current domain
!      integer,intent(in) :: isd,ied,jsd,jed
!^-- Added target to see if ps will stick around
!
!---------------------
!***  Local variables
!---------------------
!
      integer :: ncid, dimid                                            &
              , is_input, ie_input, js_input, je_input                  &
              , yaxis_1_in, xaxis_1_in, var_id                          &
              , ni, nj, bc_hour, bc_update_interval                     &
              , is_ncinput, ie_ncinput                                  &
              , js_ncinput, je_ncinput

      character(len=3)   :: int_to_char
      character(len=6)   :: fmt='(i3.3)'
      character(len=60)  :: var_name
      character(len=80)  :: source
      character(len=50)  :: file_name_lbc
      logical :: lstatus = .false.
      type(FmsNetcdfFile_t) :: Lbc_data
      integer, allocatable, dimension(:) :: pes !< Array of the pes in the current pelist

      real,dimension(:,:,:),allocatable :: tsea_input,lkmsk_input,slmsk_input &
                                           , tisfc_input, fice_input
...
!
         write(int_to_char,fmt) bc_hour
         file_name_lbc='INPUT/fvcom_f'//int_to_char//'.nc'
         if (mpp_pe() == 0) then
           write(*,"('file_name_lbc = ',a)")file_name_lbc
         endif
         ...
         if (open_file(Lbc_data , file_name_lbc, "read", pelist=pes)) then
          lstatus = global_att_exists(Lbc_data, "source")
          if(lstatus) call get_global_attribute(Lbc_data, "source", source)
          call close_file(Lbc_data)
         endif

        deallocate(pes) 
        if (.not. lstatus) then
         if (mpp_pe() == 0) write(0,*) 'INPUT LBC source not found ',lstatus,' set source=No Source Attribute'
            source='No Source Attribute'
        endif
        call mpp_error(NOTE, 'INPUT lbc_data source string: '//trim(source))
!-----------------------------------------------------------------------
!***  Open the regional BC file.
!-----------------------------------------------------------------------
!
!          write(*,*)"Finding Dimensions"
          call check(nf90_open(file_name_lbc,nf90_nowrite,ncid)) !<-- Open the netcdf file; get the file ID.
!
!  CALLS FOR AXIS NAMES/DIMENSIONS NEEDS TO BE MORE FLEXIBLE THAN USING A
!  PREDETERMINED NAME!!!
          call check(nf90_inq_dimid(ncid,'lat',dimid))!'yaxis_1',dimid))
!<-- Get the y-axis dimension's NetCDF ID.
          call check(nf90_inquire_dimension(ncid,dimid,len=yaxis_1_in))
!<-- Get the y-axis dimension's value.
!
          call check(nf90_inq_dimid(ncid,'lon',dimid))!'xaxis_1',dimid))
!<-- Get the x-axis dimension's NetCDF ID.
          call check(nf90_inquire_dimension(ncid,dimid,len=xaxis_1_in))
!<-- Get the x-axis dimension's value.
!          write(*,*)"Found Dimensions, allocating input"
!
!-----------------------------------------------------------------------
...
!
          is_ncinput = 1
          ie_ncinput = xaxis_1_in
          js_ncinput = 1
          je_ncinput = yaxis_1_in

          is_input=Atm%bd%isc!1          ! is-nhalo_data
          ie_input=Atm%bd%iec!xaxis_1_in ! ie+nhalo_data
          js_input=Atm%bd%jsc!1          ! js-nhalo_data
          je_input=Atm%bd%jec!yaxis_1_in ! je+nhalo_data
!
!          write(*,"('is_input = ',i4.1)")is_input
!          write(*,"('js_input = ',i4.1)")js_input
!          write(*,"('ie_input = ',i4.1)")ie_input
!          write(*,"('je_input = ',i4.1)")je_input
!
          allocate( tsea_input(is_ncinput:ie_ncinput,js_ncinput:je_ncinput,1)) ; tsea_input=real_snan                 !<-- Skin Temp
          allocate( lkmsk_input(is_ncinput:ie_ncinput,js_ncinput:je_ncinput,1)) ; lkmsk_input=real_snan                 !<-- Lake Mask
!          allocate( slmsk_input(is_ncinput:ie_ncinput,js_ncinput:je_ncinput,1)) ; slmsk_input=real_snan                 !<-- Land Mask
          allocate( tisfc_input(is_ncinput:ie_ncinput,js_ncinput:je_ncinput,1)) ; tisfc_input=real_snan                 !<-- Ice Temp
          allocate( fice_input(is_ncinput:ie_ncinput,js_ncinput:je_ncinput,1)) ; fice_input=real_snan                 !<-- Fractional Ice

!!!!!!!! NEED TO ADD AN ALLOCATION HERE TO STORE THE VARIABLES
!
!-----------------------------------------------------------------------
...
!
!          write(*,*)"Finding tsfc"
! IS TIME FIRST OR LAST DIMENSION READ IN BY nf90_get_var??
          var_name='tsfc' !'tsea'
          call check(nf90_inq_varid(ncid,trim(var_name),var_id))
!<-- Get this variable's ID.
          call check(nf90_get_var(ncid,var_id                               &
                                         ,tsea_input(is_ncinput:ie_ncinput  &  !<-- Fill this task's domain boundary halo.
                                         ,js_ncinput:je_ncinput,1)          &
                                         ,start=(/1,1,1/)                   & !<-- Start reading the data array here.
                                         ,count=(/ie_ncinput,je_ncinput,1/)))
!<-- Extent of data to read in each dimension.
!           write(*,*)"Finding glmask"
           var_name='glmask' !'lkmsk'
           call check(nf90_inq_varid(ncid,trim(var_name),var_id))
!<-- Get this variable's ID.
           call check(nf90_get_var(ncid,var_id                               &
                                          ,lkmsk_input(is_ncinput:ie_ncinput &  !<-- Fill this task's domain boundary halo.
                                          ,js_ncinput:je_ncinput,1)          &
                                          ,start=(/1,1,1/)                   &   !<-- Start reading the data array here.
                                          ,count=(/ie_ncinput,je_ncinput,1/)))  !<-- Extent of data to read in each dimension.

!           var_name='slmsk'
!           call check(nf90_inq_varid(ncid,trim(var_name),var_id)) !<-- Get this variable's ID.
!           call check(nf90_get_var(ncid,var_id                               &
!                                          ,slmsk_input(is_ncinput:ie_ncinput &  !<-- Fill this task's domain boundary halo.
!                                          ,js_ncinput:je_ncinput,1)          &
!                                          ,start=(/1,1,1/)                   &  !<-- Start reading the data array here.
!                                          ,count=(/ie_ncinput,je_ncinput,1/)))  !<-- Extent of data to read in each dimension.
!           write(*,*)"Finding tisfc"
           ...
           call check(nf90_inq_varid(ncid,trim(var_name),var_id)) !<-- Get this variable's ID.
           call check(nf90_get_var(ncid,var_id                               &
                                          ,tisfc_input(is_ncinput:ie_ncinput &  !<-- Fill this task's domain boundary halo.
                                          ,js_ncinput:je_ncinput,1)          &
                                          ,start=(/1,1,1/)                   &  !<-- Start reading the data array here.
                                          ,count=(/ie_ncinput,je_ncinput,1/)))  !<-- Extent of data to read in each dimension.
!           write(*,*)"Finding fice"
           ...
           call check(nf90_inq_varid(ncid,trim(var_name),var_id))  !<-- Get this variable's ID.
           call check(nf90_get_var(ncid,var_id                              &
                                          ,fice_input(is_ncinput:ie_ncinput &   !<-- Fill this task's domain boundary halo.
                                          ,js_ncinput:je_ncinput,1)         &
                                          ,start=(/1,1,1/)                  &   !<-- Start reading the data array here.
                                          ,count=(/ie_ncinput,je_ncinput,1/)))  !<-- Extent of data to read in each dimension.

!-----------------------------------------------------------------------
!*** Assign variables to object Atm
!-----------------------------------------------------------------------
!
           do ni=is_input,ie_input
            do nj=js_input,je_input
              Atm%lkmsk(ni,nj) = lkmsk_input(ni,nj,1)  !<-- Pass Lake Mask to Atm for use in initializing in atmosphere.f90
              if(lkmsk_input(ni,nj,1)==1)then               !<-- If lake mask is True (==1), then change surface temperature and land mask
                if(mpp_pe() == 0)then
!                  write(*,"('ni = ',i4.1)")ni
!                  write(*,"('nj = ',i4.1)")nj
!                  write(*,"('slmsk = ',f15.8)")slmsk_input(ni,nj,1)
!                  write(*,"('tisfc = ',f15.8)")tisfc_input(ni,nj,1)
                  write(*,"('fice = ',f15.8)")fice_input(ni,nj,1)
!                  write(*,"('lkmsk_input = ',f15.8)")lkmsk_input(ni,nj,1)
!                  write(*,"('Atm%ts B4 = ',f15.8)")Atm%ts(ni,nj)
!                  write(*,"('ps B4 = ',f30.8)")Atm%ps(ni,nj)
                endif
                if(tsea_input(ni,nj,1) <= 170.0) then ! If surface input temperature is in degrees C, convert to Kelvin
                  Atm%ts(ni,nj) = tsea_input(ni,nj,1) + 273.15
                else
                  Atm%ts(ni,nj) = tsea_input(ni,nj,1)
                endif
                if(tisfc_input(ni,nj,1) <= 170.0) then ! If ice temeprature is in degrees C, convert to Kelvin
                  Atm%tisfc(ni,nj) = tisfc_input(ni,nj,1) + 273.15
                else
                  Atm%tisfc(ni,nj) = tisfc_input(ni,nj,1)
                endif
!                Atm%fice(ni,nj) = fice_input(ni,nj,1)
                if(fice_input(ni,nj,1) >= 0.15) then ! If the fractional ice is greater than 15%, set it to ice point
                  Atm%slmsk(ni,nj) = 2  ! 2 == ice point
                  Atm%fice(ni,nj) = fice_input(ni,nj,1)
                else !Else, remove ice concentration and set to open water
                  Atm%slmsk(ni,nj) = 0  ! 0 == open water point
                  Atm%fice(ni,nj) = 0.0 !fice_input(ni,nj,1)
                endif
                if(mpp_pe() == 0)then
!                  write(*,"('tsea_input = ',f15.8)")tsea_input(ni,nj,1)
!                  write(*,"('Atm%ts After = ',f15.8)")Atm%ts(ni,nj)
!                  write(*,"('ps After = ',f30.8)")Atm%ps(ni,nj)
                  write(*,"('Atm%fice(ni,nj)=',f15.8)")Atm%fice(ni,nj)
                endif
              endif
            enddo
           enddo
!
!-----------------------------------------------------------------------
! Deallocate and close opened file
!-----------------------------------------------------------------------
!
...
!-----------------------------------------------------------------------
!
           if(allocated(tsea_input))then
             deallocate(tsea_input)
           endif
           if(allocated(lkmsk_input))then
             deallocate(lkmsk_input)
           endif
!           if(allocated(slmsk_input))then
!             deallocate(slmsk_input)
!           endif
           if(allocated(tisfc_input))then
             deallocate(tisfc_input)
           endif
           if(allocated(fice_input))then
             deallocate(fice_input)
           endif
!
      end subroutine read_lowbc_data
!-----------------------------------------------------------------------
!
!-----------------------------------------------------------------------
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
