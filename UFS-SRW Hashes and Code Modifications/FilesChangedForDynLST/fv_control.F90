...
!--------------------------------------------------------------------
! Lower bc update options
! Added by David.M.Wright
!--------------------------------------------------------------------
     logical, pointer :: lowbcupdate ! T/F to update lower boundary conditions
     integer, pointer :: lowbc_int   ! Hours between updates to lower boundary condition
     character(len=128), pointer :: lowbc_dir ! Directory to find lower boundary conditions
     character(len=128), pointer :: lowbc_base ! File base name for lower boundary conditions
...
!David.M.Wright
     call read_namelist_lowbc_nml(Atm(this_grid)) !read in low bc update namelist
#ifdef MULTI_GASES
...
!David.M.Wright, Added the next 4 namelist flags for lower bc updating
       lowbcupdate                   => Atm%flagstruct%lowbcupdate
       lowbc_int                     => Atm%flagstruct%lowbc_int
       lowbc_dir                     => Atm%flagstruct%lowbc_dir
       lowbc_base                    => Atm%flagstruct%lowbc_base
!
...
!David.M.Wright
!Subroutine to read in lowbc updating namelist entries
     subroutine read_namelist_lowbc_nml(Atm)

       type(fv_atmos_type), intent(inout) :: Atm
       integer :: f_unit, ios, ierr
       !  local version of these variables to allow PGI compiler to compile
       character(len=128) :: lowbc_dir = ''
       character(len=128) :: lowbc_base  = ''
       logical :: lowbcupdate
       integer :: lowbc_int

       namelist /lowbc_nml/ lowbcupdate, lowbc_int, lowbc_dir, lowbc_base

       ! Read Main namelist
       read (input_nml_file,lowbc_nml,iostat=ios)
       ierr = check_nml_error(ios,'lowbc_nml')

       call write_version_number ( 'FV_CONTROL_MOD', version )
       unit = stdlog()
       write(unit, nml=lowbc_nml)

       !Basic option processing
       Atm%flagstruct%lowbcupdate = lowbcupdate
       Atm%flagstruct%lowbc_int = lowbc_int
       if (len_trim(lowbc_dir) /= 0) Atm%flagstruct%lowbc_dir = lowbc_dir
       if (len_trim(lowbc_base) /= 0) Atm%flagstruct%lowbc_base = lowbc_base

     end subroutine read_namelist_lowbc_nml
