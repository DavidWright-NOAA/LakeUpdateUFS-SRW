...
use fv_regional_mod,    only: start_regional_restart, read_new_bc_data, &
                              ...
                              update_lowbc
!Modification above by David.M.Wright
...
!
   if(Atm(n)%flagstruct%regional)then
     ...
     call read_new_bc_data(Atm(n), Time, Time_step_atmos, p_split, &
                           isd, ied, jsd, jed )
!
!*** Modifications to update lower bc (specifically lake temperatures) by
!David.M.Wright
!
     if(Atm(n)%flagstruct%lowbcupdate)then
       call update_lowbc(Atm(n), Time, Time_step_atmos, p_split)
     endif
   endif
...
! log(pe) <-- prsik
     ...
     do k = 1, npz
       ...
       do ix = 1, blen
         i = Atm_block%index(nb)%ii(ix)
         j = Atm_block%index(nb)%jj(ix)
! Added by David.M.Wright, ADD IN ICE THICKNESS AND ZORL?
!  If lowbcupdate = True, then read in tsfc from Atm
!         write(*,*)"Checking for lowbcupdate"
         if(Atm(mygrid)%flagstruct%lowbcupdate .and. Atm(mygrid)%lkmsk(i,j) ==1) then
!           write(*,*)"lowbcupdate is True"
!           if( i == 82 .and. j == 261) then
!             write(*,"('BEFORE Lake H. Sfcprop%tsfco(ix) = ',f15.8)")IPD_Data(nb)%Sfcprop%tsfco(ix)
!             write(*,"('BEFORE Lake H. Sfcprop%tsfc(ix) = ',f15.8)")IPD_Data(nb)%Sfcprop%tsfc(ix)
!             write(*,"('BEFORE Lake H. Sfcprop%tisfc(ix) = ',f15.8)")IPD_Data(nb)%Sfcprop%tisfc(ix)
!             write(*,"('BEFORE Lake H. Sfcprop%slmsk(ix) = ',f15.8)")IPD_Data(nb)%Sfcprop%slmsk(ix)
!             write(*,"('BEFORE Lake H. Sfcprop%fice(ix) = ',f15.8)")IPD_Data(nb)%Sfcprop%fice(ix)
!             write(*,"('BEFORE Lake H. Atm(mytile)%fice(i,j) = ',f15.8)")Atm(mytile)%fice(i,j)
!           endif
           IPD_Data(nb)%Sfcprop%tsfco(ix) = _DBL_(_RL_(Atm(mygrid)%ts(i,j)))
           IPD_Data(nb)%Sfcprop%tsfc(ix) = _DBL_(_RL_(Atm(mygrid)%ts(i,j)))
           IPD_Data(nb)%Sfcprop%tisfc(ix) = _DBL_(_RL_(Atm(mygrid)%tisfc(i,j)))
           IPD_Data(nb)%Sfcprop%slmsk(ix) = _DBL_(_RL_(Atm(mygrid)%slmsk(i,j)))
           IPD_Data(nb)%Sfcprop%fice(ix) = _DBL_(_RL_(Atm(mygrid)%fice(i,j)))
           if(Atm(mygrid)%slmsk(i,j) == 2) then
             IPD_Data(nb)%Sfcprop%zorl(ix) = _DBL_(_RL_(0.1))
           endif
!           if( i == 82 .and. j == 261) then
!             write(*,"('AFTER Lake H. Sfcprop%tsfco(ix) = ',f15.8)")IPD_Data(nb)%Sfcprop%tsfco(ix)
!             write(*,"('AFTER Lake H. Sfcprop%tsfc(ix) = ',f15.8)")IPD_Data(nb)%Sfcprop%tsfc(ix)
!             write(*,"('AFTER Lake H. Sfcprop%tisfc(ix) = ',f15.8)")IPD_Data(nb)%Sfcprop%tisfc(ix)
!             write(*,"('AFTER Lake H. Sfcprop%slmsk(ix) = ',f15.8)")IPD_Data(nb)%Sfcprop%slmsk(ix)
!             write(*,"('AFTER Lake H. Sfcprop%fice(ix) = ',f15.8)")IPD_Data(nb)%Sfcprop%fice(ix)
!             write(*,"('AFTER Lake H. Sfcprop%fice(ix) = ',f15.8)")IPD_Data(nb)%Sfcprop%fice(ix)
!             write(*,"('AFTER Lake H. Atm(mytile)%fice(i,j) = ',f15.8)")Atm(mytile)%fice(i,j)
!           endif
         endif
!
